...
void* filesystem::mv(vector<wstring> params) { //params[0] - old, params[1] - new
	if (params.size() == 2) {
		...
		//проверка
		int index = findbyname(fs, path_old); //если это не за пределами, если файл с этим индексом файл
		if (index != fs.size() + 1) {
			//если существует путь, куда перемещаем файл
			//if
